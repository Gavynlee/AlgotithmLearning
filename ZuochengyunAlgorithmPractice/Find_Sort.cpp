 ...
 *
 * step 1: 建初堆.
 *         要将一个无序序列调整为堆，就必须将其所对应的完全二叉树中以每一结点为根的子树都调
 *         整为堆。显然， 只有一个结点的树必是堆，而在完全二叉树中，所有序号大千L1/2」的结点都是
 *         叶子，因此以这些结点为根的子树均已是堆。这样，只需利用筛选法，从最后一个分支结点L1/2」
 *         开始，依次将序号为L1/2」、L1/2」-1 … 、1的结点作为根的子树都调整为堆即可。
 * step2: 筛选法调整堆.
 *        从r[2s]和r[2s+1]中选出关键字较大者，假设r[2s]的关键字较大，比较r[s]和r[2s]的关键字。
 *        若r[s].key> = r[2s].key, 说明以r[s]为根的子树已经是堆，不必做任何调整。
 *        若r[s].key<r[2s].key, 交换r[s]和r[2s]。交换后，以r[2s+1]为根的子树仍是堆，如果以
 *        r[2s]为根的子树不是堆，则重复上述过程，将以r[2s]为根的子树调整为堆，直至进行到叶子结点为止。
 */
//大顶堆，调整堆
//A为存储堆的数组，len为数组长度（堆中元素个数），k为待检查的节点下标
void AdjustHeap(int A[], int len, int k)
{
    A[0] = A[k]; // 注意堆顶元素是从A[1]开始的
    // 沿着key较大的孩子节点向下调整
